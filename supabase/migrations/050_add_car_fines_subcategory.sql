-- Migration 050: Add standalone "Fines & Violations" category group with subcategories
-- Also removes fines_penalties from miscellaneous and car_fines from transport (if exists)
...
BEGIN
  ...
  LOOP
    ...
    IF EXISTS (
      ...
      -- Group exists, just ensure subcategories
      SELECT id INTO _gid FROM category_groups
        WHERE user_id = _uid AND taxonomy_key = 'fines' AND retired_at IS NULL LIMIT 1;
    ELSE
      -- Create the fines group
      INSERT INTO category_groups (id, user_id, name, icon, color, sort_order, taxonomy_key)
      VALUES (gen_random_uuid(), _uid, 'Fines & Violations', 'siren', _color, 95, 'fines')
    ...
    -- Retire old fines_penalties from miscellaneous (if it exists)
    UPDATE categories
    SET retired_at = NOW()
    WHERE user_id = _uid
      AND taxonomy_key = 'fines_penalties'
      AND retired_at IS NULL;

    -- Retire car_fines from transport (if it exists from a prior attempt)
    UPDATE categories
    SET retired_at = NOW()
    WHERE user_id = _uid
      AND taxonomy_key = 'car_fines'
      AND retired_at IS NULL;
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM categories WHERE user_id = _uid AND taxonomy_key = _tk AND retired_at IS NULL
  ) THEN RETURN; END IF;

  IF EXISTS (
    SELECT 1 FROM categories WHERE user_id = _uid AND taxonomy_key = _tk AND retired_at IS NOT NULL
  ) THEN
    UPDATE categories
