...
// Match: *1234 / **1234 / 1234* / ending 1234 / ينتهي بـ 1234.
// No trailing \b — would block matches when the asterisk is followed by
// a non-word char like \n.
const MASK_RE = /\*+\s*(\d{4})(?!\d)|(\d{4})\s*\*+|\bending\s+(\d{4})\b|ينتهي\s+ب?ـ?\s*(\d{4})|(?:رقم|no\.?)\s+(\d{4})(?!\d)/gi;

const ROLE_PATTERNS: Array<{ re: RegExp; role: Last4Hit['role'] }> = [
  // ORDER MATTERS — most specific first.
  { re: /(من|from)\s+حساب/i,                          role: 'from' },
  { re: /(من|from)\s+بطاقة/i,                         role: 'from' },
  { re: /(الى|إلى|to)\s+حساب/i,                       role: 'to' },
  { re: /(الى|إلى|to)\s+بطاقة/i,                      role: 'to' },
  { re: /(?:^|[\s\n])(الى|إلى|to)(?=[\s\n])/i,        role: 'to' },
  { re: /(?:^|[\s\n])(من|from)(?=[\s\n])/i,           role: 'from' },
  // Channel keywords ARE card indicators. The Arabic forms accept the
  // ة/ت/تك/تي/تك/تكم suffixes (بطاقة|بطاقتك|بطاقتي|...) common in SMS.
  { re: /(بطاق[ةت]\w*|card|apple\s?pay|google\s?pay|mada|مدى|stc\s?pay|urpay)/i, role: 'card' },
  { re: /(حساب\w*|account|الايبان|iban)/i,            role: 'account' },
];
